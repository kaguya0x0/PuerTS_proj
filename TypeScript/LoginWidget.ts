...
let loginUI = uiManager.OpenWidget(world,"login") as UE.Game.Blueprints.WBP_Login.WBP_Login_C;
...
loginUI.Button_Login.OnClicked.Add(() => {
    ...
    if(LoginData.CheckID(userID)){
        if(LoginData.LoginTest(userID,userPassword)){
            console.log("登录成功");
...
loginUI.Button_EnrollSubmit.OnClicked.Add(() => {
    ...
    LoginData.AddUserInfo(enrollID,enrollPassword);
...
function LoginSuccess(userid:string):UE.UserWidget{
    let userUI = uiManager.OpenWidget(world,"userInfo") as UE.Game.Blueprints.WBP_UserInfo.WBP_UserInfo_C;
